...
erpnext.accounts.taxes.setup_tax_validations("Purchase Requisition");
...
frappe.ui.form.on("Purchase Requisition", {
	...
	onload: function (frm) {
		...
		// On cancel and amending a Purchase Requisition with advance payment, reset advance paid amount
		if (frm.is_new()) {
...
erpnext.buying.PurchaseOrderController = class PurchaseOrderController extends (
		...
		if (doc.docstatus == 1) {
			...
			if (!["Closed", "Delivered"].includes(doc.status)) {
					...
					if (!this.frm.doc.__onload || this.frm.doc.__onload.can_update_items) {
						this.frm.add_custom_button(__("Update Items"), () => {
							erpnext.utils.update_child_items({
								frm: this.frm,
								child_docname: "items",
								child_doctype: "Purchase Requisition Detail",
					...
					if (flt(doc.per_billed) < 100 || flt(doc.per_received) < 100) {
						if (doc.status != "On Hold") {
							this.frm.add_custom_button(
								__("Hold"),
								() => this.hold_purchase_requisition(),
								__("Status")
							);
						} else {
							this.frm.add_custom_button(
								__("Resume"),
								() => this.unhold_purchase_requisition(),
								__("Status")
							);
						}
						this.frm.add_custom_button(
							__("Close"),
							() => this.close_purchase_requisition(),
							__("Status")
						);
					}
				}
				...
				if (this.frm.has_perm("submit")) {
					this.frm.add_custom_button(
						__("Re-open"),
						() => this.unclose_purchase_requisition(),
						__("Status")
					);
				}
			}
	...
	get_items_from_open_material_requests() {
		erpnext.utils.map_current_doc({
			method: "erpnext.stock.doctype.material_request.material_request.make_purchase_requisition_based_on_supplier",
			args: {
	...
	make_inter_company_order(frm) {
		frappe.model.open_mapped_doc({
			method: "erpnext.buying.doctype.purchase_requisition.purchase_requisition.make_inter_company_sales_order",
			frm: frm,
		});
	}
	...
	make_purchase_receipt() {
		frappe.model.open_mapped_doc({
			method: "erpnext.buying.doctype.purchase_requisition.purchase_requisition.make_purchase_receipt",
			frm: cur_frm,
	...
	make_purchase_invoice() {
		frappe.model.open_mapped_doc({
			method: "erpnext.buying.doctype.purchase_requisition.purchase_requisition.make_purchase_invoice",
			frm: cur_frm,
		});
	}
	...
	make_subcontracting_order() {
		frappe.model.open_mapped_doc({
			method: "erpnext.buying.doctype.purchase_requisition.purchase_requisition.make_subcontracting_order",
			frm: cur_frm,
	...
	add_from_mappers() {
		...
		this.frm.add_custom_button(
			...
			function () {
				erpnext.utils.map_current_doc({
					method: "erpnext.stock.doctype.material_request.material_request.make_purchase_requisition",
					source_doctype: "Material Request",
					target: me.frm,
					setters: {
		...
		this.frm.add_custom_button(
			...
			function () {
				erpnext.utils.map_current_doc({
					method: "erpnext.buying.doctype.supplier_quotation.supplier_quotation.make_purchase_requisition",
	...
	unhold_purchase_requisition() {
		cur_frm.cscript.update_status("Resume", "Draft");
	}

	hold_purchase_requisition() {
		var me = this;
...
cur_frm.cscript.update_status = function (label, status) {
	frappe.call({
		method: "erpnext.buying.doctype.purchase_requisition.purchase_requisition.update_status",
...
frappe.ui.form.on("Purchase Requisition", "is_subcontracted", function (frm) {
	if (frm.doc.is_old_subcontracting_flow) {
